...
set -euo pipefail
cd "$(dirname "$0")/.."   # -> audio/
REPO=..   # repo root: addon/, addon-testtone/, addon-skipback/ live there
...
"$ZIG" cc -target "$TARGET" -shared -fPIC -O2 -s \
    ...
    -o "$REPO/addon/forceAudioJack.so" src/forceAudioJack.c -lpthread -lrt
...
exported=$(readelf --dyn-syms -W "$REPO/addon/forceAudioJack.so" \
...
"$ZIG" cc -target "$TARGET" -O2 -s \
    -o "$REPO/addon-testtone/injectTone" src/injectTone.c -lpthread -lrt -lm

echo "== skipbackHost (Skipback extraction-ring consumer) =="
"$ZIG" cc -target "$TARGET" -O2 -s \
    -o "$REPO/addon-skipback/skipbackHost" src/skipbackHost.c -lpthread -lrt -lm

chmod 0755 "$REPO/addon/forceAudioJack.so" "$REPO/addon-testtone/injectTone" "$REPO/addon-skipback/skipbackHost"
ls -la "$REPO/addon/forceAudioJack.so" "$REPO/addon-testtone/injectTone" "$REPO/addon-skipback/skipbackHost"

# nodeServer's moduler scans exactly one NSMODULE.json per AddOns/* folder,
# so the test tone and skipbackHost each get their own optional folder
# rather than living inside addon/ (AddOns/ForceShadow, the core install).
echo "== done -> addon/ (AddOns/ForceShadow), addon-testtone/ (AddOns/ForceShadowTestTone), addon-skipback/ (AddOns/ForceAudioJackSkipback) =="
